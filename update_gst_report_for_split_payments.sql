-- Update GST Breakup Report Stored Procedure to handle split payments
-- This script updates usp_GetGSTBreakupReport to correctly aggregate multiple payment lines per order

USE [RestaurantDB];  -- Update with your database name
GO
...
BEGIN
    ...
    SELECT 
        ...
        -- Taxable Value = Total Amount (incl GST) - GST - Discount
        -- For split payments: sum Amount from all lines, subtract total discount (only on first line), subtract GST
        SUM(ISNULL(p.Amount,0)) + SUM(ISNULL(p.TipAmount,0)) - SUM(ISNULL(p.GSTAmount,0)) - SUM(ISNULL(p.DiscAmount,0)) AS TaxableValue,
        ...
        -- Invoice Total = Sum of all payment amounts + tips (this is what customer actually paid)
        SUM(ISNULL(p.Amount,0)) + SUM(ISNULL(p.TipAmount,0)) AS InvoiceTotal
    INTO #OrderGST
    FROM Orders o
...
PRINT 'GST Breakup Report stored procedure updated successfully to handle split payments.';
PRINT 'The report now correctly aggregates multiple payment lines per order as a single invoice.';
GO
